  ...
  return <div className="rrView">
    ...
    <div className="rrSplit">
      <section className="rrList" aria-labelledby="rr-list-heading">
        ...
        <div className="rrTableViewport" tabIndex={0} role="region" aria-label="Risk register table, scroll horizontally for all columns"><table className="rrTable"><thead><tr>{['Risk','Owner','CIA','Risk Score Path','Rating','Appetite','Lifecycle','Treatment','Review','Actions'].map(name => <th scope="col" key={name}>{name}</th>)}</tr></thead><tbody>{visible.map(risk => <tr key={risk.id} className={selected?.id === risk.id ? 'rrSelectedRow' : ''} onClick={() => setSelection(risk.id)}><td><button type="button" className="rrRiskSelect" aria-pressed={selected?.id === risk.id} onClick={() => setSelection(risk.id)}>{risk.title}</button><small>{label(risk.category)} · Inherent {scoreLabel(risk, risk.inherentScore)}</small></td><td>{risk.owner}</td><td><CiaBadges risk={risk}/></td><td aria-label={`Inherent ${scoreLabel(risk, risk.inherentScore)}, current residual ${scoreLabel(risk, risk.residualScore)}, target ${targetRiskScore(risk) ?? 'not set'}`}>{scoreLabel(risk, risk.inherentScore)} &rarr; {scoreLabel(risk, risk.residualScore)} &rarr; {scoreLabel(risk, targetRiskScore(risk))}</td><td><Badge variant={residualRating(risk).toLowerCase() === 'critical' ? 'danger' : residualRating(risk).toLowerCase() === 'high' ? 'warning' : 'default'} size="sm">{validScore(risk.residualScore) === null ? 'Not set' : residualRating(risk)}</Badge></td><td><Badge variant={tone(risk.appetiteStatus)} size="sm">{TOLERANCE_STATUS_LABELS[risk.appetiteStatus]}</Badge></td><td>{RISK_STATUS_LABELS[risk.status]}</td><td><Badge variant={tone(risk.treatmentStatus || '')} size="sm">{label(risk.treatmentStatus || 'not_started')}</Badge></td><td>{label(risk.reviewStatus || 'not_reviewed')}</td><td><button type="button" className="rrRowAction" aria-label={`View full details for ${risk.title}`} onClick={e => { e.stopPropagation(); props.onSelectRisk(risk); }}>View</button></td></tr>)}</tbody></table></div>
      ...
      <aside className="rrDetail" aria-label="Selected risk detail">
        ...
        {selected ? <><div className="rrDetailTitle"><h3>{selected.title}</h3><Badge variant="default" size="sm">{label(selected.category)}</Badge></div><p className="rrDetailSubtitle">Inherent score {Math.round(selected.inherentScore)}</p>
          ...
          <div className="rrPlans">{plans.length ? plans.map(plan => <button type="button" className="rrPlan" key={plan.id} onClick={() => props.onEditTreatment(plan)}><strong>{plan.title}</strong><span>{plan.linkedControls?.length ? `${plan.linkedControls.length} controls linked` : 'No controls linked'}</span><span>{plan.linkedControls?.slice(0,2).map(link => link.title || link.controlId).join(', ')}</span><span>Expected residual after treatment: {plan.expectedResidualScore ?? 'Not set'}</span><span>{plan.owner} · {dateLabel(plan.dueDate)}</span><span className="rrPlanProgress"><progress value={plan.progressPercent} max={100} aria-label={`${plan.title} progress`}/>{plan.progressPercent}%</span><Badge variant={tone(plan.status)} size="sm">{label(plan.status)}</Badge></button>) : <p className="rrEmptyPlan">No treatment plans recorded for this risk. Use Record Treatment to add one.</p>}</div>
