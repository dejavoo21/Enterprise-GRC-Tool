...
import { getRiskSeverity, getRiskSeverityLabel, normalizeCiaImpacts, RISK_STATUS_LABELS } from '../types/risk';
...
export function RiskRegisterKpis({ risks, openPlans }: { risks: RiskIntelligenceRiskSummary[]; openPlans: number }) {
  const summaries = [
    ...
    { name: 'Critical risks', value: risks.filter(r => getRiskSeverity(r.residualScore) === 'critical').length, note: 'Current residual rating', icon: <RiskIcon size={20}/>, tone: 'danger' },
...
export function RiskRegisterView(props: RiskWorkspaceViewProps) {
  ...
  const filters: Filter[] = [
    ...
    { id: 'rating', name: 'ratings', value: props.selectedRating, options: options(['critical','high','medium','low']), change: value => props.setSelectedRating(value as RiskSeverity | 'all') },
    ...
    { id: 'lifecycle', name: 'lifecycle statuses', value: props.selectedRiskStatus, options: Object.entries(RISK_STATUS_LABELS), change: value => props.setSelectedRiskStatus(value as RiskStatus | 'all') },
  ...
  const selected = visible.find(r => r.id === selection) || visible[0] || null;
  ...
  return <div className="rrView">
    ...
    <div className="rrSplit">
      <section className="rrList" aria-labelledby="rr-list-heading">
        <header className="rrListHeader"><div><h2 id="rr-list-heading">Risk Register <span>({sorted.length} risks)</span></h2><p>Select a risk to view details, treatment plans, and related information.</p></div><label className="rrSort">Sort by<select value={sort} onChange={e => setSort(e.target.value)}><option value="source">Register order</option><option value="title">Risk name</option><option value="residual">Highest residual</option><option value="owner">Owner</option></select></label></header>
        <div className="rrTableViewport" tabIndex={0} role="region" aria-label="Risk register table, scroll horizontally for all columns"><table className="rrTable"><thead><tr>{['Risk','Owner','CIA','Residual','Rating','Appetite','Lifecycle','Treatment','Review','Actions'].map(name => <th scope="col" key={name}>{name}</th>)}</tr></thead><tbody>{visible.map(risk => <tr key={risk.id} className={selected?.id === risk.id ? 'rrSelectedRow' : ''} onClick={() => setSelection(risk.id)}><td><button type="button" className="rrRiskSelect" aria-pressed={selected?.id === risk.id} onClick={() => setSelection(risk.id)}>{risk.title}</button><small>{label(risk.category)} · Inherent {Math.round(risk.inherentScore)}</small></td><td>{risk.owner}</td><td><CiaBadges risk={risk}/></td><td>{Math.round(risk.residualScore)}</td><td><Badge variant={risk.residualScore >= 20 ? 'danger' : risk.residualScore >= 12 ? 'warning' : 'default'} size="sm">{getRiskSeverityLabel(risk.residualScore)}</Badge></td><td><Badge variant={tone(risk.appetiteStatus)} size="sm">{TOLERANCE_STATUS_LABELS[risk.appetiteStatus]}</Badge></td><td>{RISK_STATUS_LABELS[risk.status]}</td><td><Badge variant={tone(risk.treatmentStatus || '')} size="sm">{label(risk.treatmentStatus || 'not_started')}</Badge></td><td>{label(risk.reviewStatus || 'not_reviewed')}</td><td><button type="button" className="rrRowAction" aria-label={`View full details for ${risk.title}`} onClick={e => { e.stopPropagation(); props.onSelectRisk(risk); }}>View</button></td></tr>)}</tbody></table></div>
      ...
      <aside className="rrDetail" aria-label="Selected risk detail">
        <header><h2>Selected Risk</h2>{selected && <button type="button" className="rrTextButton" onClick={() => props.onSelectRisk(selected)}>View full details ↗</button>}</header>
        {selected ? <><div className="rrDetailTitle"><h3>{selected.title}</h3><Badge variant="default" size="sm">{label(selected.category)}</Badge></div><p className="rrDetailSubtitle">Inherent score {Math.round(selected.inherentScore)}</p>
          <dl className="rrSnapshot"><div><dt>Owner</dt><dd>{selected.owner || 'Not assigned'}</dd></div><div><dt>Rating</dt><dd><Badge variant={selected.residualScore >= 20 ? 'danger' : 'warning'} size="sm">{getRiskSeverityLabel(selected.residualScore)}</Badge></dd></div><div><dt>Appetite</dt><dd><Badge variant={tone(selected.appetiteStatus)} size="sm">{TOLERANCE_STATUS_LABELS[selected.appetiteStatus]}</Badge></dd></div><div><dt>Residual score</dt><dd>{Math.round(selected.residualScore)}</dd></div><div><dt>CIA impacts</dt><dd><CiaBadges risk={selected}/></dd></div><div><dt>Lifecycle status</dt><dd>{RISK_STATUS_LABELS[selected.status]}</dd></div><div><dt>Treatment status</dt><dd>{label(selected.treatmentStatus || 'not_started')}</dd></div><div><dt>Next review</dt><dd>{dateLabel(selected.nextReviewDate)}</dd></div><div><dt>Strategy</dt><dd>{selected.treatmentStrategy ? label(selected.treatmentStrategy) : 'Not set'}</dd></div><div><dt>Treatment due</dt><dd>{dateLabel(selected.treatmentDueDate)}</dd></div></dl>
          <div className="rrPlansHeader"><h3>Treatment Plans ({plans.length})</h3><button type="button" className="rrTextButton" onClick={() => props.onNavigate('treatments')}>View all</button></div>
          <div className="rrPlans">{plans.length ? plans.map(plan => <button type="button" className="rrPlan" key={plan.id} onClick={() => props.onEditTreatment(plan)}><strong>{plan.title}</strong><span>{plan.owner} · {dateLabel(plan.dueDate)}</span><span className="rrPlanProgress"><progress value={plan.progressPercent} max={100} aria-label={`${plan.title} progress`}/>{plan.progressPercent}%</span><Badge variant={tone(plan.status)} size="sm">{label(plan.status)}</Badge></button>) : <p className="rrEmptyPlan">No treatment plans recorded for this risk. Use Record Treatment to add one.</p>}</div>
          <div className="rrDetailActions"><Button variant="primary" onClick={() => props.onEditRisk(selected)}>Edit Risk</Button><Button variant="secondary" onClick={() => props.onCreateTreatment(selected)}>Record Treatment</Button><button type="button" className="rrSecondaryButton rrFull" onClick={() => props.onNavigate('treatments')}>Open Treatment Plans</button><button type="button" disabled title="Control linking is not available yet">Link Control · Coming soon</button><button type="button" disabled title="Evidence linking is not available yet">Link Evidence · Coming soon</button></div>
