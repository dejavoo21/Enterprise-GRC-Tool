...
test('real risk/treatment repositories and methodology route on disposable core schema', { skip: !url }, async () => {
  ...
  try {
    ...
    const historicalAfter = (await client.query("SELECT * FROM risks WHERE id='historical-unclassified'")).rows[0];
    const { risk_ref: historicalRef, ...historicalFields } = historicalAfter;
    assert.match(historicalRef, /^RSK-\d{4,}$/);
    assert.deepEqual(historicalFields, historicalBefore, 'Reference backfill must not change scores, pinning, timestamps or CIA');
    await client.query(referenceMigration);
    assert.equal((await client.query("SELECT risk_ref FROM risks WHERE id='historical-unclassified'")).rows[0].risk_ref, historicalRef);
    await assert.rejects(client.query("UPDATE risks SET risk_ref='RSK-FAKE' WHERE id='historical-unclassified'"), /immutable/);
    ...
    assert.match(legacy.riskRef!, /^RSK-\d{4,}$/);
    assert.notEqual(legacy.riskRef, historicalRef);
    const concurrent = await Promise.all(Array.from({length:4},(_,index)=>risks.createRisk('other',{...input,title:`Concurrent reference ${index}`})));
    assert.equal(new Set(concurrent.map(row=>row.riskRef)).size,4,'Concurrent inserts receive unique references');
    const edited = await risks.updateRisk('other',concurrent[0].id,{title:'Updated title'});
    assert.equal(edited?.riskRef,concurrent[0].riskRef,'Reference survives normal risk edits');
