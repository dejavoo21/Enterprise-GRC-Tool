...
export async function list(workspaceId: string, riskId?: string): Promise<RiskTreatmentPlan[]> {
  const result = await query(`SELECT p.*, r.title AS risk_title FROM risk_treatment_plans p JOIN risks r ON r.id = p.risk_id AND r.workspace_id = p.workspace_id WHERE p.workspace_id = $1 ${riskId ? 'AND p.risk_id = $2' : ''} ORDER BY p.updated_at DESC`, riskId ? [workspaceId, riskId] : [workspaceId]);
...
export async function get(workspaceId: string, id: string): Promise<RiskTreatmentPlan | null> {
  const result = await query(`SELECT p.*, r.title AS risk_title FROM risk_treatment_plans p JOIN risks r ON r.id = p.risk_id AND r.workspace_id = p.workspace_id WHERE p.workspace_id = $1 AND p.id = $2`, [workspaceId, id]);
...
export async function update(workspaceId: string, id: string, input: Partial<RiskTreatmentPlan>): Promise<RiskTreatmentPlan | null> {
  ...
  for (const [key, column] of Object.entries(fields)) if (input[key as keyof RiskTreatmentPlan] !== undefined) { values.push(input[key as keyof RiskTreatmentPlan] || null); sets.push(`${column} = $${values.length}`); }
  ...
  if (!sets.length) return get(workspaceId, id);
  await query(`UPDATE risk_treatment_plans SET ${sets.join(', ')}, updated_at = NOW() WHERE id = $1 AND workspace_id = $2`, values);
